<template>
    <div class="relative">
        ...
        <div v-show="isOpen" id="statusDropdown"
            ...
            <ul class="space-y-2 text-[14px]" aria-labelledby="statusDropdownButton">
                <li v-for="statusOption in statusOptions" :key="statusOption.id" class="flex items-center">
                    ...
                    <label :for="statusOption.id" class="ml-2 text-[14px] font-medium text-gray-900 dark:text-gray-100">{{
                        statusOption.name }}</label>
                </li>
            </ul>
            <button @click="saveStatuses" class="mt-2 w-full py-2 px-4 text-[14px] font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 focus:outline-none focus:ring-4 focus:ring-primary-500 dark:bg-primary-500 dark:hover:bg-primary-600 dark:focus:ring-primary-700">
                Save
            </button>
        </div>
    </div>
